...
| **[Ein UI](https://ui.eindev.ir/)** | — | 19+ 元件，shadcn 相容、Radix UI 基礎、Tailwind CSS v4、內建深色模式、完整 TypeScript、WCAG 無障礙 | **極高** — 與現有技術棧完美契合（shadcn + Radix + Tailwind） |
...
|------|-------------|--------|---------|
| `backdrop-filter: blur()` | 完整支援 | 完整支援 | 完整支援 |
| `backdrop-filter: saturate()` | 完整支援 | 完整支援 | 完整支援 |
| SVG `feDisplacementMap` 折射 | 支援 | **不支援** | **不支援** |
| `inset` box-shadow | 完整支援 | 完整支援 | 完整支援 |

**結論**: Phase 1-3 的純 CSS 方案在所有主流瀏覽器均可運作。僅在考慮使用 `liquid-glass-react` 的 SVG 折射模式時需提供 Chromium-only 的增強體驗 + 其他瀏覽器的優雅降級。
...
| 舊瀏覽器不支援 | 低 | 低 | `@supports` 查詢 + 實色 fallback |
